...
六组，全部固定 token 预算 N=256（= G0 的 token 数，于是每组都在同一预算上）：
...
两个必须赢的对照：**G4 vs G3**（度量 4D vs 图像网格，弱命题）、
**G4 vs M2**（一致 vs 错配，**强命题，标题句的唯一直接证据**）。
前提检验用 **G2 vs G0**，不是 G1 vs G0——后者差了 8 倍 token。

⚠️ **M2 是主实验，不是消融**：G4 > G3 只说明度量坐标好用，
G4 > M2 才说明"两者一致"本身有价值。正文 Table 1 必须包含它。
...
同一 (h,w) 的 K 帧 patch，G4 比 G3 多分开 **28.5%** 的格子（已扣掉时间轴，
G3 作配对参照），槽位利用率两组齐平（240 vs 242）。
...
完整计划、六组对照的判读规则与排期见 **`docs/06-后续计划.md`**。
