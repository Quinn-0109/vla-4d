...
五组对照，每一对只差一个变量（`docs/06` §3）：

| 对照 | 隔离出什么 |
|---|---|
| G1 vs G0 | K=8 全量 vs K=1 —— 历史帧到底有没有用 |
| G3 vs G2 | patch 级 vs 帧级池化 —— 粒度（对上 4D-VLA） |
| **G4 vs G3** | **度量 4D 坐标 vs 图像网格坐标 —— 核心创新，唯一必须赢的对照** |

**G4 vs G3 排在 11 月而不是最后**：打平就是创新点不成立，要留出转向时间。
