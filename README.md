...
K=1 LoRA 微调 step 20 000 达 **72.0%**，对官方同批次 **76.0%**（p=0.65, n=50）。
训练流程能复现官方水平，冻结视觉主干未构成障碍。`docs/05` §8

顺带的方法论产出：训练曲线 8%(5k) → 58%(10k) → **56%(15k)** → 72%(20k)。
非单调不是模型退化，是 **n=50 检不出 25 个点以下的差异**。功效分析见 `docs/05` §8.3。

### 🔄 阶段 3：4D 池化主线（当前）

> **2026-08 路径重写。** 上一版铺了六项前置、五组对照、双相机、单目深度、
> 种子方差（~110 h），主线被淹没。已收敛到**五次训练**（~81 h，含强命题所需的错配臂），
