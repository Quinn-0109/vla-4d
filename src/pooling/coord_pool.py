...
⚠️ 已知代价：分箱均值池化正是 docs/04 §4.4 判为"只作 baseline"的那类算子。
...
def _resolve_bins(
    ...
    **现在的理由：时间与空间之间没有可比的"物理跨度"。** 二分搜索的前提是
    ...
    ③ docs/06 §1.3① 的"同一 (h,w) 不同帧可能是不同表面"这个**错误合并机制
...
def _fps_assign(qi: torch.Tensor, budget: int, n_t: int,
    ...
    **G4 只用到 63/256 槽而 G3 用了 242**（⚠️ 那是**贪心箱数规则**下的数，
    换成二分搜索后真值深度实测是 240/242 —— 这段动机因此比当初以为的弱得多）。
    FPS 按构造把中心放在有内容的地方，
...
def coord_bin_pool(
    ...
    （docs/06 §3.0.4 ①）——M2 的 (t̄,h̄,w̄) 与 G4 的 (t̄,x̄,ȳ,z̄) 由同一份代码算出，
    这样两组的池化侧才是逐位相同的。
...
if __name__ == "__main__":
    ...
    ⚠️ **FPS + Voronoi 试过，被自检否掉了**（partition='fps' 仍保留可复现）。
       ...
       **FPS 帮 G4、害 G3** —— 抬高的那部分与坐标系的信息量无关，是算子与
       ...
       推向点密处（度量坐标 242 → 256 槽）。

       ⚠️ 这里曾写「真值深度下原本只有 63」—— **错了,那是贪心箱数规则时代的数**,
       与均匀/等量的边界无关。真值深度实测（冻结包围盒，150 窗口）：
       G4 均匀体素 **240**、等量分箱 **256**；G3 两者都是 242。
       等量分箱的真实增益是 240→256（约 6%），不是"拿回四分之三预算"。""")

    print("""
⚠️ 两处只在真实深度下才有意义，别拿上面的数当结论：

  ① ~~G4 的槽位利用率明显低于 G3~~ —— **真值深度实测后基本不成立**。
     2026-09 用冻结包围盒、150 个真实窗口重测（`scripts/depth_bins.py`）：

         坐标            均匀体素   等量分箱
         G3 (t,h,w)        242       242     ← 逐位相同，等量分箱对 G3 是恒等的
         G4 (t,x,y,z)      240       256

     ⚠️ 长期挂在这里的 **63/256 是贪心箱数规则时代的数**，与均匀/等量的边界
     无关（见 `_resolve_bins`：贪心卡在占用数的平台上）。二分搜索早就把它修到
     240 了，而那笔功劳一度被记在等量分箱头上。等量分箱的真实增益是
     240 → 256（约 6%）—— 仍然值得要（零成本、网格上逐位等价），但**不是**
     "拿回四分之三预算"。
