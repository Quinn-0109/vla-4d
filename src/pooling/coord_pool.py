...
⚠️ 两处只在真实深度下才有意义，别拿上面的数当结论：
  ...
  ① G4 的槽位利用率明显低于 G3。均匀体素在度量空间里浪费掉大半预算
     （**真值深度实测 G4 仅 63/256**，合成薄壳上 242），因为可见 patch 贴在
     一层薄壳上、立方格大半是空的。这个浪费会直接压低 G4，**而它不报任何错**。
     等量分箱（现在的默认）把箱边推向点密处，合成深度下 242 → 256；
     FPS 也能补满，但在网格坐标上有惩罚，已否掉（见上面 [1]）。
     ⚠️ **真值深度下的利用率还没重测** —— 63/256 那个数是均匀体素时代的，
     等量分箱在真实深度上到底补到多少，是开工前最后一件没做的事。
